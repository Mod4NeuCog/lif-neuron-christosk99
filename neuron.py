...
#program depends on processing speed to be fast enough for stupid line80 to work. Adjust 0.1[better]-0.15[worse] accordingly.
...
class Neuron():
    ...
    def time_alive(self):
        while self.alive == True:
            ...
            print(self.V_hist[-1])
            #really inaccurate way to make this work
            amt = 0.15 - (time.time()-curr_time)
...
def main():
    ...
    plt.title("livegraph neuron")
    plt.xlabel("time window (0.2s)")
    plt.ylabel("V")
    ax.set_ylim(-85.,-55.)
